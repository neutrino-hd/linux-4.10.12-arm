...
static unsigned char brcmstb_clk_sw_get_parent(struct clk_hw *hw)
{
	return 0;
}

static int brcmstb_clk_sw_enable(struct clk_hw *hw)
{
	int i;
	for (i = 0; i < clk_hw_get_num_parents(hw); i++) {
		...
		if (parent) {
			clk_enable(parent->clk);
		}
	}
	return 0;
}

static void brcmstb_clk_sw_disable(struct clk_hw *hw)
{
	int i;
	for (i = 0; i < clk_hw_get_num_parents(hw); i++) {
		struct clk_hw *parent;
		parent = clk_hw_get_parent_by_index(hw, i);
		if (parent) {
			clk_disable(parent->clk);
		}
	}
}

static int brcmstb_clk_sw_prepare(struct clk_hw *hw)
{
	int i;
	for (i = 0; i < clk_hw_get_num_parents(hw); i++) {
		struct clk_hw *parent;
		parent = clk_hw_get_parent_by_index(hw, i);
		if (parent) {
			clk_prepare(parent->clk);
		}
	}
	return 0;
}

static void brcmstb_clk_sw_unprepare(struct clk_hw *hw)
{
	int i;
	for (i = 0; i < clk_hw_get_num_parents(hw); i++) {
		struct clk_hw *parent;
...
static const struct clk_ops brcmstb_clk_sw_ops =
{
	...
	.enable = brcmstb_clk_sw_enable,
	.disable = brcmstb_clk_sw_disable,
	.prepare = brcmstb_clk_sw_prepare,
	.unprepare = brcmstb_clk_sw_unprepare,
};
...
static void of_brcmstb_clk_divider_setup(struct device_node *node)
{
	struct clk *clk;
	const char *clk_name = node->name;
	void __iomem *reg;
...
static const struct of_device_id brcmstb_clk_match_full[] __initconst = {
	...
	{ .compatible = "divider-clock",
	  .data = of_brcmstb_clk_divider_setup, },
	{}
};
